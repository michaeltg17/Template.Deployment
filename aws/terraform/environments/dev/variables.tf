...
variable "db_master_password" {
  description = "RDS master password. Stored in the <env>-db-master secret (and, when db_app_username is empty, in <env>-db). Never in GitHub"
  type        = string
  sensitive   = true
}

variable "db_app_username" {
  description = "Dedicated app login (prod). Empty (dev/qa) = the app connects as the master user"
  type        = string
  default     = ""
}
...
variable "image_api_key" {
  description = "Image API key for this environment (stored in the <env>-image-api secret)"
  type        = string
  sensitive   = true
}
